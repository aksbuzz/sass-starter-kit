...
import { AuthService, type OAuthProvider } from '../../services/auth.service.js'
import { TenantService }                   from '../../services/tenant.service.js'
import { authenticate }                     from '../../hooks/authenticate.js'
import { requirePlatformAdmin }             from '../../hooks/require-platform-admin.js'
import { REFRESH_COOKIE_OPTIONS }           from '../../plugins/jwt.js'
...
export const authRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post('/auth/impersonate', {
    ...
    schema: {
      tags:     ['auth'],
      summary:  'Start impersonating a user (platform admin only)',
      security: [{ bearerAuth: [] }],
      body: {
        type:                 'object',
    ...
    handler: async (
      ...
      // Prevent impersonation of other platform admins
      ...
      // Issue tokens for the impersonation session
      const accessPayload: Omit<AccessTokenPayload, 'iat' | 'exp'> = {
        purpose: 'access',
        sub:     targetUserId,
  ...
  fastify.post('/auth/stop-impersonation', {
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const data = await withAdmin(async ({ repos }) => {
        ...
        return { adminSession, adminSessionData: adminSession.data as SessionData }
      })

      // Issue new tokens for the admin's original session
      const accessPayload: Omit<AccessTokenPayload, 'iat' | 'exp'> = {
        purpose: 'access',
