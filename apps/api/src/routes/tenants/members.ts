...
export const membersRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  // Email job enqueued transactionally — only dispatched after commit.
  fastify.post('/tenants/me/members/invite', {
    ...
    handler: async (
      ...
      const invitation = await tenantSvc.inviteMember(request.ctx!, request.body)
  ...
  fastify.patch('/tenants/me/members/:membershipId/role', {
    ...
    handler: async (
      ...
      const membership = await tenantSvc.updateMemberRole(
        request.ctx!,
  ...
  fastify.delete('/tenants/me/members/:membershipId', {
    ...
    handler: async (
      ...
      await tenantSvc.removeMember(request.ctx!, request.params.membershipId)
      return reply.code(204).send()
    },
  })
  ...
  // Requires the user to be authenticated (so we know who is accepting).
  fastify.post('/invitations/:token/accept', {
    ...
    handler: async (
      ...
      const result = await tenantSvc.acceptInvitation(
        request.ctx!.userId,
        user.email,
      ...
      return reply.code(201).send({
        ...
        // Frontend should call POST /auth/workspace with this tenantId to get a new token
